...
# 🌈 MVVM에서의 화면의 분리와 접근
화면을 분리 파일이 클경우에 파일을분리해서 사용

폼서밋테스트 화면을 예시로 만약에 화면에 소스가 길어지고 복잡해졌다고 가정하면 
디테일부분을 따로 띄어서 화면을 하나 또 만들수도있고  화면을 분리해서
requires 로 가져올수도있다 
그런데 이제 생각해야할게 MVVM 아키텍처에서 화면을 분리를 했을경우에 
나타나는 영향이 무엇이있느냐 생각을해봐야한다 
화면이여러개가있지만 우리는 중앙에 탭을만들어놓고 각각의 화면을 호출하였다
우리는 폼서밋이든  AJAX 스토어테스트든 각각의 다른파일로 나누고 MVVM 으로 구축을하였다
그러면 메인에서 이 폼서밋테스트 화면에 그리드 의 한행에 접근을할수있을것인가 없을것인가를 ㅅ
생각을해봐야한다 마찬가지로 밑에있는 detail 폼 부분을 파일하나 따로 뺏다면
그럴경우에 요 폼서밋 테스트에서 requires 그 폼부분을 접근할수 있느냐 없느냐를 구분을해야한다

정답을먼저말하자면 

접근을할수있을떄가있고 할수없을떄가있다 

할수있을때는 뭐냐면 단순히 뷰로써 뷰로만 딱 requires 
